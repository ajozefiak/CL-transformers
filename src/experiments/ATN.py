...
class DataLoader:
    def __init__(self, B, T, tokens, key):
...
# ds is the dataset
# task is the task index from 0 to num_tasks-1
# task_len is (B * num_time_steps_per_epoch) == art_per_task
def get_tokens_next_task(ds, task, task_len):
    idx_start = task * task_len
    idx_end = (task+1) * task_len
    return ds[idx_start:idx_end]['tokens_129']


# We replace N, number of tokens, with art_per_task: the number of articles (examples) per task
# Originally, we used B = 16, T = 129 in our experiments in Colab
# We are now moving to the scale of Permuted Shakespeare experiment => B = 8, T = 129
def run_experiment_ATN(config, alg, alg_params, ds, B, T, art_per_task, epochs, tasks, seed, save_neuron_ages, save_results, save_path, verbose, print_freq, save_weights, save_weights_freq):
