...
# Add model config
def run_experiment_PS_112024_cold_start(config, alg, alg_params, text, B, T, N, epochs, tasks, seed, save_neuron_ages, save_results, save_path, verbose, print_freq, save_weights, save_weights_freq):
    ...
    for task in range(tasks):
        ...
        # New Code: Cold start the model at beginning of each new task
        task_random_key, task_split_key = jr.split(task_random_key)
        train_state, train_step = get_transformer_methods(config, alg, alg_params, task_split_key)
        neuron_ages = init_neuron_ages(config)
        # End new task. 
