...
for seed in seeds:
    ...
    plt.title(f"Loss on Final Epoch, Seed {seed}")
    plt.ylabel("Loss")
    plt.xlabel("Task")
    plt.show()
    plt.legend()
    plt.savefig(path_root + path_subroot+ f"seed_{seed}_avg_loss_final_epoch.png")
    plt.clf()
    ...
    plt.title(f"Cumulative Average Loss on Final Epoch, Seed {seed}")
    plt.xlabel("Task")
    plt.ylabel("Cumulative Average Loss")
    plt.legend()
    plt.show()
    plt.savefig(path_root + path_subroot+ "seed_{seed}_cumulative_avg_loss_final_epoch.png")
    plt.clf()
    ...
    plt.plot(cumulative_avg_L2, label="L2")
    plt.plot(cumulative_avg_reset, label="Reset-L2")
    plt.title(f"Cumulative Average Loss over All Time Steps, Seed {seed}")
    plt.xlabel("Time Step")
    plt.ylabel("Cumulative Average Loss")
    plt.legend()
    plt.show()
    plt.savefig(path_root + path_subroot+ "seed_{seed}_cumulative_avg_loss.png")
    plt.clf()
